...
async function printItem(item) {
  ...
  new_element.innerHTML = `
        <div class="container-item-default">
            <div id="image-${item.fs_id}" class="container-image"><img src="${
    item.image
  }" onclick="zoom(this)"></div>
            <div class="container-info">
                <span id="title-${item.fs_id}" class="title">${item.name}</span>
                <div class="container-button">
                    <div id="container-download-${
                      item.fs_id
                    }" class="container-download-button">
                        <button id="get-download-${
                          item.fs_id
                        }" type="button" class="download-button">Download ${convertToMB(
    item.size
  )} MB</button>
                    </div>
                    <div class="${
                      item.type == "video"
                        ...
                        <button id="stream-${
                          item.fs_id
                        }" type="button" class="stream-button"><i class="fa-solid fa-play"></i></button>
                    </div>
                </div>
            </div>
        </div>
  ...
  const downloadButton = new_element.querySelector(
    `#get-download-${item.fs_id}`
  );
  ...
  if (response.status == "success") {
    ...
    Object.entries(downloadLinks).forEach(([key, value], index) => {
      ...
      new_element.addEventListener("click", () =>
        startDownload(new_element.value)
      );
    });
  } else {
...
async function getURLStream(fs_id, dlink = null) {
  ...
  try {
    ...
    if (response.status == "success") {
      ...
      const stream_url = old_url
        .replace(old_domain, "kul-ddata")
